...
| VEML7700 (custom)                                             | courtyard  | Converted footprint, so nothing is inherited from a trusted library: courtyard extended to enclose the pads, every pad re-derived from the Vishay drawing. Matches to within 5 µm.                                                                                  |
| Tactile switches (custom)                                     | —          | Four big pads, wide spacing, no fine pitch. The only real risk is the internal A-B / C-D pairing, verified against the XKB TS-1187A drawing.                                                                                                                     |
...
Companion to **`Footprint_Check_ESP32S3_PlantMonitor_RevA.md`** · ESP32-S3 Plant Monitor Rev A · both files live in `docs/`
